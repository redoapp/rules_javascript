...
var textwrap = { wrap, fill, dedent };

var argparse = createCommonjsModule(function (module) {
...
const repr = util__default["default"].inspect;
...
function _alias(object, from, to) {
    try {
        ...
        Object.defineProperty(object, from, {
            value: util__default["default"].deprecate(object[to], sub('%s.%s() is renamed to %s.%s()',
                name, from, name, to)),
            enumerable: false
        });
    } catch {}
...
function deprecate(id, string) {
    _deprecations[id] = _deprecations[id] || util__default["default"].deprecate(() => {}, string);
    _deprecations[id]();
}
...
function _AttributeHolder(cls = Object) {
    ...
    return class _AttributeHolder extends cls {
        ...
        toString() {
            return this[util__default["default"].inspect.custom]()
        }
...
const HelpFormatter = _camelcase_alias(_callable(class HelpFormatter {
    ...
    _dedent() {
        this._current_indent -= this._indent_increment;
        assert__default["default"](this._current_indent >= 0, 'Indent decreased below 0.');
        this._level -= 1;
    }
    ...
    _format_usage(usage, actions, groups, prefix) {
        ...
        // if optionals and positionals are available, calculate usage
            ...
            if (prefix.length + usage.length > text_width) {
                ...
                assert__default["default"](opt_parts.join(' ') === opt_usage);
                assert__default["default"](pos_parts.join(' ') === pos_usage);
    ...
    * _iter_indented_subactions(action) {
    ...
    _split_lines(text, width) {
        ...
        let textwrap$1 = textwrap;
        return textwrap$1.wrap(text, { width })
    }

    _fill_text(text, width, indent) {
        text = text.replace(this._whitespace_matcher, ' ').trim();
        let textwrap$1 = textwrap;
        return textwrap$1.fill(text, { width,
...
const FileType = _callable(class FileType extends Function {
    ...
    call(string) {
        ...
        let fd;
        try {
            fd = fs__default["default"].openSync(string, this._flags, this._options.mode);
        } catch (e) {
        ...
        if (this._flags.includes('r')) {
            return fs__default["default"].createReadStream(undefined, options)
        } else if (this._flags.includes('w')) {
            return fs__default["default"].createWriteStream(undefined, options)
        } else {
    ...
    [util__default["default"].inspect.custom]() {
        ...
        let kwargs = Object.entries(this._options).map(([ k, v ]) => {
            if (k === 'mode') v = { value: v, [util__default["default"].inspect.custom]() { return '0o' + this.value.toString(8) } };
            return [ k, v ]
        });
        let args_str = []
...
const _ActionsContainer = _camelcase_alias(_callable(class _ActionsContainer {
    ...
    constructor() {
        ...
        ;[ 'storeConst', 'storeTrue', 'storeFalse', 'appendConst' ].forEach(old_name => {
            ...
            this.register('action', old_name, util__default["default"].deprecate(this._registry_get('action', new_name),
...
const ArgumentParser = _camelcase_alias(_callable(class ArgumentParser extends _AttributeHolder(_ActionsContainer) {
    ...
    constructor() {
        ...
        if (prog === undefined) {
            prog = path__default["default"].basename(get_argv()[0] || '');
        }
        ...
        this.register('type', 'string',
            util__default["default"].deprecate(String, 'use {type:"str"} or {type:String} instead of {type:"string"}'));
        // end
    ...
    _parse_known_args(arg_strings, namespace) {
        ...
        let consume_optional = start_index => {
            ...
            assert__default["default"](action_tuples.length);
    ...
    _read_args_from_files(arg_strings) {
        ...
        for (let arg_string of arg_strings) {
            ...
            // replace arguments referencing files with the file content
                ...
                try {
                    let args_file = fs__default["default"].readFileSync(arg_string.slice(1), 'utf8');
                    let arg_strings = [];
